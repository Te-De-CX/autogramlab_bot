...
async def handle_deadline(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(confirmation + "\n\nYou can expect a response within few hours.", reply_markup=get_main_menu_keyboard())
    return ConversationHandler.END
...
async def send_admin_summary(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(chat_id=config.ADMIN_CHAT_ID, text=admin_message, parse_mode='Markdown')
    except:
        await context.bot.send_message(chat_id=config.ADMIN_CHAT_ID, text=admin_message.replace('*', '').replace('`', ''))
